...
## 33. The `trend` book — a universe A/B beside the policy A/B (2026-09-04)

Third book per user, alongside `control` and `autoswap`. It is the first that
varies **which symbols may be bought** rather than what is done with them:
same signal stream, same days, same admission rules, but it may only buy names
in `input/trend.txt` (104 curated trending symbols, seeded from the ad-hoc
`input/trending.txt` TradingView export).

### Architecture: filter the stream, do not run a second scan
The obvious build — a second `breakout_scanner.py` run over the trend list,
writing its own signal CSVs — was rejected. **92 of the 104 trend symbols are
already in `all.txt`**, so the 9:35 scan already produces their signals; a
second scan would re-derive them at a different instant, adding scan cost and a
timing confound for no coverage gain. Filtering the existing stream keeps both
arms' signals generated by the same code at the same moment, so the ONLY
difference between control and trend is eligibility — which is the variable
under test.
...
scan input. `tests/test_trend_book_universe.py` fails if it ever stops being
tracked.

### The four gaps the policy-only A/B never had to handle
The registry was already generic across `/portfolio/books`, the Streamlit
selector, the mobile `BookProvider`/switcher, both Compare views,
`dev_dashboard`, `fork_books`, `refresh_prices_all_users` and the exit
evaluator — so **a third entry surfaces end-to-end with zero UI or mobile code
changes** and inherits ATR trailing stops, stop-breach auto-close and exit
alerts. The actual work was:

1. **`_book_universe()` fails CLOSED.** A configured watchlist that is missing
   or parses to zero symbols returns an EMPTY set (buy nothing), never `None`
   (buy anything). The file is gitignored and lives on a docker volume seeded
   once, so "missing in production" is a real operational state — and failing
   open would silently turn the variant into a second copy of control while the
   comparison still looked healthy. Same shape as every §22/§23 bug. `None` and
   empty must stay distinguishable; they mean opposite things.
2. **`seed`** — `seed=None` starts the book empty at $100k instead of cloning
   control. `ensure_forked` hardcoded control as the clone source, which would
   have opened the trend book holding names its own watchlist does not contain.
   `autoswap` keeps `seed='control'` — a policy A/B's arms MUST start identical.
3. 🔴 **`ensure_forked` no longer re-stamps control's fork record when a LATER
   book is forked.** The old condition (`date != fork_date or force`) re-stamped
   control on every fork. Control's stamp is the `since` date `book_compare`
   measures every one of its metrics from, so **adding this book would have
   silently restarted the running control-vs-autoswap comparison from today**,
   discarding its history since 2026-08-11 with no error anywhere. Control is now
   stamped only on its first-ever fork, or on a forced re-fork of the book it is
   already paired with (the deliberate `fork_books --force` restart).
4. **`book_compare` differences over the variant's own window.** Control's
   since-inception return minus a three-week-old book's return compares two
   different stretches of market. Books sharing a fork date (control/autoswap)
   are unaffected.

Plus `advise_swaps: False` — `_run_swap_stage` runs for every book regardless of
`auto_swap` (control *advises* over Telegram), and Telegram still has no
per-book routing, so a third advising book is a third daily message.

`skipped_universe` is the new readout: rows that cleared every quality gate and
were rejected only for being off-universe. It is the number that says whether
the book is actually behaving as a variant. Verified read-only against a real
production file (`signals_swing_20260903`): 4 signals clear the quality gate,
control considers all 4, trend considers 1 (`NOW`), `skipped_universe = 3`.

### Second instance of the §26 plain-assignment leak — found by this work
`debug_memory_scan._stub_network()` sets `ap._compute_atr_pct = lambda s: 3.0`
by plain assignment and was never restored. The balance guard reads that as a
**300% ATR**, so once `test_memory_trace.py` had run, the portfolio-ATR risk cap
blocked **every admission in every later test in the session** — silently. The
new tests failed in the full suite while passing alone, and looked like a broken
feature rather than a polluted session.

§26 fixed `_sandbox`'s leak and added `restore_sandbox_patches`, but never
covered `_stub_network`'s six patches. The fixture now restores all of them and
clears the three caches both functions seed. `test_every_module_level_patch_is_
restored` is the structural half: it AST-walks both functions for `mod.attr = …`
assignments and fails if any is absent from the fixture, so a third instance
...
```bash
ssh -i "$SSH_KEY" ubuntu@"$ORACLE_HOST"
cd ~/stocksBreakout && git pull --ff-only && \
  docker compose up -d --build api scanner-cron dashboard   # --build is mandatory (§5a)
docker compose cp ./input/trend.txt scanner-cron:/app/input/trend.txt
docker compose exec scanner-cron python3 -c \
  "import auto_portfolio as ap; print(len(ap._book_universe('trend')))"   # expect 104
docker compose exec scanner-cron python3 fork_books.py --dry-run   # then without
```
The `input_data` volume is seeded from the image ONCE, so a rebuild does not
pick up a new watchlist — the `cp` is required. ⚠ Run `fork_books.py` **on the
box only**: the local `users.db` is the stale Mac artifact and would resurrect a
