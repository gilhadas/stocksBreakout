...
UPDATED for the control-vs-autoswap A/B: each user now holds one book per entry
in `ap.BOOKS`, and refresh must cover EVERY (user, book) pair — a book whose
prices are never refreshed has no trail raised and no stop honoured, which is
the same defect this file was written for, one dimension over. Result keys stay
bare emails for the control book so existing log-scraping is unaffected, and
gain a " [book]" suffix for variants.
"""
...
def test_refreshes_every_user_not_just_default():
    ...
    def fake_refresh(user_id=None, book=None):
        seen.append((user_id, book))
    ...
    expected = [(u.id, b) for u in users for b in ap.BOOKS]
    assert seen == expected, "must refresh every (user, book) pair"
    # Control keeps the bare email key; variants are suffixed.
    assert set(result) == {
        (u.email if b == ap.DEFAULT_BOOK else f'{u.email} [{b}]')
        for u in users for b in ap.BOOKS
    }
